...
export default defineConfig({
  ...
  use: { baseURL: remote || 'http://127.0.0.1:8121', serviceWorkers: 'block',
    trace: 'retain-on-failure', screenshot: 'only-on-failure' },
  projects: [{ name: 'chromium', use: { browserName: 'chromium' } },
    { name: 'firefox', use: { browserName: 'firefox' } },
    { name: 'webkit', use: { browserName: 'webkit' } }],
  webServer: remote ? undefined : { command: `${process.env.FACTORY_PYTHON || '.venv/bin/python'} -m uvicorn app.main:app --host 127.0.0.1 --port 8121`,
    url: 'http://127.0.0.1:8121/health', reuseExistingServer: false, timeout: 20000 },
});
